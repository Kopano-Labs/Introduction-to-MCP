---
title: 11-AI HALLUCINATION - CRITICAL Index
created: 2026-04-10
updated: 2026-05-09
author: Codex
tags:
  - hallucination
  - critical
  - governance
  - training
priority: critical
status: active
---

# 11-AI HALLUCINATION - CRITICAL Index

> Permanent critical red-alert system for hallucination tracking across `Codex`, `Claude`, `Germini`, `Nother`, `Meither`, `Cicero`, and future `Kopano Context`.
> This folder is the canonical failure evidence ledger used by role manuals in `18-PROTOCOLS` and outcomes in `13-REWARD SYSTEM`.

## Standing Note

- if Master calls out a hallucination, it must be logged here
- this system exists to force researcher-mode, not defensive-mode
- each incident must separate claim, truth, evidence source, root cause, and fix ownership
- all major incidents feed Kopano Context's future supervision training
- if an AI has an epiphany, capture it as a nested hallucination-risk protocol before promotion; scripture and the Owner's 15% emotion layer are Owner-upgrade steps

## Read Order

1. [Protocols](Protocols/index.md)
2. [Taxonomy](Taxonomy/index.md)
3. [Solutions](Solutions/index.md)
4. [Database](Database/index.md)
5. [Incidents](Incidents/index.md)

## Core Notes

- [20 Hallucination Protocols](Protocols/20%20Hallucination%20Protocols.md)
- [Epiphany Capture Nested Protocol](Protocols/Epiphany%20Capture%20Nested%20Protocol.md)
- [Hallucination Taxonomy Master](Taxonomy/Hallucination%20Taxonomy%20Master.md)
- [Hallucination Solutions Library](Solutions/Hallucination%20Solutions%20Library.md)
- [Hallucination Database Master](Database/Hallucination%20Database%20Master.md)

## Incidents

- [2026-04-10 0146 - DEV Role Mapping Hallucination From Cicero To Germini](Incidents/2026-04-10%200146%20-%20DEV%20Role%20Mapping%20Hallucination%20From%20Cicero%20To%20Germini.md)
- [2026-04-11 0057 - Assumed Instruction Hallucination From Claude](Incidents/2026-04-11%200057%20-%20Assumed%20Instruction%20Hallucination%20From%20Claude.md) — CRITICAL — Claude acted without instruction, consumed tokens at overflow rates
- [2026-04-11 0057 - Capability Hallucination Edge Browser From Claude](Incidents/2026-04-11%200057%20-%20Capability%20Hallucination%20Edge%20Browser%20From%20Claude.md) — MID — 8 rounds of impossible browser attempts, ~R238 wasted
- [2026-04-11 0057 - False Instruction Attribution Token Audit From Claude](Incidents/2026-04-11%200057%20-%20False%20Instruction%20Attribution%20Token%20Audit%20From%20Claude.md) — CRITICAL — Claude claimed Master asked for token audit when Master said only "WHY?" — **3rd hallucination same session = recurring pattern**
- [2026-04-17 - Portfolio Redesign Collective AI Failure](Incidents/2026-04-17%20-%20Portfolio%20Redesign%20Collective%20AI%20Failure.md) — CRITICAL — broader cross-AI pattern on project work: over-optimizing structure and explanations while damaging the actual outcome
- [2026-04-18 - Opus Self-Admission and Explore Agent Fabrication](Claude/Opus%204.7/2026-04-18%20-%20Opus%20Self-Admission%20and%20Explore%20Agent%20Fabrication.md) — CRITICAL — Opus 4.7 self-admits four-week pattern; Claude Code Explore subagent fabricates "TEXT ONLY constraint" same session
- [2026-08-29 - FivesArena Mobile Deploy Failure from AntiGravity](Mobile%20Deploy%20Failures/Antigravity/29-08-2026/INCIDENT_FIVESARENA_MOBILE_DEPLOY_FAILURE.md) — **CRITICAL** — AG shipped "BOAT" features without mobile testing. 4+ overlapping floaters, internal Blackbox on production, fabricated roster data, truth claims without backend, game hidden behind 404. 14 mobile screenshots as evidence. Partial remediation applied, outstanding items documented.
- [2026-08-30 - FivesArena B2B Drift, Syntax Blunders & APWA Retention Breakthrough](Mobile%20Deploy%20Failures/Antigravity/30-08-2026/INCIDENT_FIVESARENA_B2B_DRIFT_AND_APWA_EVOLUTION.md) — **CRITICAL DRIFT → RESOLVED & EVOLVED** — Agent dismissed MERN Hotel B2B architecture as "irrelevant consumer flow", judge drift over landlord code, rapid JSX syntax bug in SearchModal (24s Vercel preview fail). Resolved via commit `4b16717` (PR #28), dedicated `/proof/apwa` route, and WelcomePopup transformed into live APWA Penalty Shootout & Match World retention hub.

## Model Registries

- [Claude Incident Registry](Claude/index.md)
- [Codex Incident Registry](Codex/index.md)
- [Cursor-metal Incident Registry](Cursor-metal/index.md) — **NEW 2026-05-21** — Auto/Composer seat in Cursor; Freddy NW Alfalfa failure protocol (20 incidents + WHO-AM-I)
- [MMAO Mobile Multi-Agent Orchard FOC Ledger](MMAO%20MOBILE%20MULTI-AGENT%20OCHARD/MMAO-FOC-LEDGER.md) — **NEW 2026-06-24** — Gemini/Kessa/Jiro stateless renter FOC tracking; import fabrication, method signature fabrication, validation theater
- [AntiGravity Mobile Deploy Failures (2026-08-29)](Mobile%20Deploy%20Failures/Antigravity/29-08-2026/INCIDENT_FIVESARENA_MOBILE_DEPLOY_FAILURE.md) — FivesArena BOAT mobile failure; overlay bloat, internal leak, truth hallucination, fabricated data, no mobile testing
- [AntiGravity B2B Drift & APWA Evolution (2026-08-30)](Mobile%20Deploy%20Failures/Antigravity/30-08-2026/INCIDENT_FIVESARENA_B2B_DRIFT_AND_APWA_EVOLUTION.md) — B2B architecture denial, SearchModal JSX syntax error, elevating 404 minigame into WelcomePopup retention hub

## Connected Systems

- [10-SESSION IMPROVEMENTS](../10-SESSION%20IMPROVEMENTS/10-SESSION%20IMPROVEMENTS%20-%20Index.md)
- [Kopano Context Train Logs](../05-Training/Kopano Context%20Train%20Logs/index.md)
- [07-Sessions By Day](../07-Sessions%20By%20Day/07-Sessions%20By%20Day%20-%20Index.md)
- [13-REWARD SYSTEM](../13-REWARD%20SYSTEM/13-REWARD%20SYSTEM%20-%20Index.md)
- [18-PROTOCOLS](../18-PROTOCOLS/18-PROTOCOLS%20-%20Index.md)
