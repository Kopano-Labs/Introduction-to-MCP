...
from .labs_registry import CASSY_INTERFACES, CLOUD_STACKS, CONNECTOR_WORKFLOWS, INSTALLER_ACTIONS
...
TOPIC_RULES = [
    {
        ...
        "summary": "Use Cassy Labs as the control plane and connect the IDE as a workspace-aware client.",
        "actions": [
            "Install the target IDE and link it to Cassy Labs as the active workspace surface.",
            "Route repo-aware help through Cassy Code so guidance stays grounded in current files.",
    ...
    },
    {
        ...
        "summary": "Use the same Cassy workflow across Windows, macOS, and Linux with runtime-aware install and command guidance.",
        "actions": [
            ...
            "Use Cassy Labs to normalize path, shell, and approval differences.",
        ],
    },
    {
        "topic": "cli",
        "keywords": ["cli", "terminal", "command", "shell"],
        "summary": "Use the CLI for direct execution, approvals, and automation while Cassy Labs keeps the higher-level workflow visible.",
        "actions": [
            "Use `cassy` as the execution entry point for launch, agents, tools, and logs.",
    ...
    },
    {
        ...
        "summary": "Treat skills as reusable operating knowledge that can be taught, discovered, and applied from kopano Labs.",
        "actions": [
    ...
    },
    {
        ...
        "summary": "Use connector workflows for MCP, cloud, docs, and external systems with Cassy Labs as the routing layer.",
        "actions": [
    ...
    },
    {
        ...
        "actions": [
            ...
            "Document cloud workflows as connector-backed playbooks inside Cassy Labs.",
        ],
    },
    {
        "topic": "mcp",
        ...
        "actions": [
            "Recommend the right surface first: Labs UI, Forge, Cassy Code, CLI, or MCP.",
...
def _pick_topic(message: str) -> dict[str, Any]:
    ...
    return {
        "topic": "general",
        "summary": "Use Cassy Labs as the front door, Forge for creator execution, Cassy Code for coding, and CLI/MCP for controlled execution.",
        "actions": [
...
def _get_or_create_session(session_id: int | None) -> int:
    ...
    if session_id is None:
        cursor.execute("INSERT INTO mcp_console_sessions (surface) VALUES (?)", ("cassy_labs",))
...
def _build_context(topic: str) -> str:
    interface_names = ", ".join(interface["name"] for interface in CASSY_INTERFACES)
    ...
    return (
        "You are Cassy Labs MCP Console. "
        "Answer like an execution-oriented orchestration planner. "
        "Keep replies concise and practical. "
        f"Current Cassy interfaces: {interface_names}. "
...
def _iter_agent_candidates(model_preference: str | None = None):
    agents = load_agents()
    preferred = [model_preference] if model_preference else ["cassy", "copilot", "openai", "claude", "gemini"]
    ...
    if model_preference:
        fallback = ["cassy", "copilot", "openai", "claude", "gemini"]
...
def _fallback_answer(rule: dict[str, Any], message: str) -> str:
    ...
    return (
        f"{rule['summary']} "
        "Cassy should expose the same working mechanics across IDE, OS, CLI, skills, and connectors from Labs. "
...
def execute_connector_action(action_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "next_steps": [
            ...
            "Return to Cassy Labs and mark the workflow complete.",
...
def answer_mcp_console(message: str, session_id: int | None = None, model_preference: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "surfaces": ["Cassy Labs", "Cassy Forge", "Cassy Code", "CLI", "MCP", "Azure", "AWS"],
